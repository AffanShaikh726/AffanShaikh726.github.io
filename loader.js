document.addEventListener("DOMContentLoaded", () => {
    ...
    if (w < 1000) {
        ...
        if (/iPad|iPhone|iPod/.test(navigator.userAgent)) {
            ...
            for (i = 0; i < colm.length; i++) {
                colm[i].style.paddingBottom = "100px";
            }
        } else {
    ...
    window.addEventListener("resize", () => {
        ...
        if (w < 1000) {
            ...
            for (i = 0; i < toLeft.length; i++) {
                toLeft[i].style.textAlign = "center";
            }
        }
    });
});
...
function fnChangeTheme() {
    ...
    let abtme = document.querySelector(".readMoreText");
    //.css var
    ...
    if (rootStyle.getPropertyValue("--number") == 0) {
        ...
        abtme.classList.remove("text-darken-4");
        ...
        abtme.classList.add("text-darken-4");
